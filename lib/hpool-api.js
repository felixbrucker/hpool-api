...
class HpoolApi {
  constructor(apiKey, userName, connectionCookie) {
    if (!apiKey) {
    ...
    if (!connectionCookie) {
      throw new Error('No connectionCookie supplied!');
    }
  ...
  async getPendingBalance() {
    ...
    return parseFloat(result.receiving_account_balance);
  }
